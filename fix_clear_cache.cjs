...
if (!code.includes('const [showClearCacheConfirm, setShowClearCacheConfirm] = useState(false);')) {
  code = code.replace(
...
// Modify hotkey handler to open confirmation dialog
code = code.replace(
  '        handleClearCache();\n      }',
...
// Oh wait, Logout is in Header.tsx, wait let's find a good place in AdminDashboard.tsx to put this modal.
// Maybe before the very end of return
